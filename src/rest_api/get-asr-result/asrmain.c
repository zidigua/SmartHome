...
#ifdef DEBUG
	#define dbug_printf(format,...) printf(format,##__VA_ARGS__)
#else
...
static RETURN_CODE fill_config(struct asr_config *config) {
    ...
    char api_key[] = "TPqAHVQEmVZ9G5scZOhMcKvb";
    // 填写网页上申请的APP SECRET 如 $secretKey="94dc99566550d87f8fa8ece112xxxxx"
    char secret_key[] = "1QGUB8lkaekGwXGlFyXBTG3oN4hUhGHY";
    // 需要识别的文件
    char *filename = "./result/rec.wav";
...
int  get_result_point(char *result)
{
	char *str_start,*str_end;
	str_start = strstr(result,"result\":[\"");
	if(str_start == NULL){
		printf("解析结果头失败\n");
		return -1;
	}
	str_end = strstr(result,"\"]");
	if(str_end == NULL){
		printf("解析结果尾失败\n");
		return -1;
	...
	FILE * fp;
	fp = fopen("./result/result.txt","wb");
	if(fp == NULL){
		dbug_printf("open file error\n");
		return -1;
	}

	fwrite(str_start + 10, 1, ((str_end-2) - (str_start+11)), fp);
	fclose(fp);
...
RETURN_CODE run_asr(struct asr_config *config, const char *token) {
    ...
    dbug_printf("request url :%s\n", url);
    dbug_printf("header is: %s\n", header);
